...
def main():
  ...
  for single_con in cursor_con:
    ...
    dataCon = dt.datetime.utcfromtimestamp(single_con["Start_Time"]).strftime("%Y-%m-%d")
    
    try:
        date_index = record_date_list.index(dataCon)
        record_conBytes_list[date_index] += (single_con["A2Bbytes"]+single_con["B2Abytes"])
    except ValueError:
        record_date_list.append(dataCon)
  ...
  for i in range(len(record_date_list)):      #node
    ...
    if(check_loop_first == 1):
      fdata_d3js.write("\t{\"Date\": \"")
      fdata_d3js.write(record_date_list[i])
      fdata_d3js.write("\", \"conBytes\": ")
