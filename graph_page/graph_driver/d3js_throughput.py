...
def main():
  ...
  for single_con in cursor_con:
    ...
    if(flow_type == "tcp"):
      ...
      if(single_con["Connection_Type"] == 17):                        #UDP
        ...
        try:
          date_index = record_date_list.index(data_time)
          record_flow_list[date_index] += (single_con["A2Bbytes"])
          record_pktNum_list[date_index] += (single_con["A2Bpacket"])
        except ValueError:
          record_date_list.append(data_time)
          record_flow_list.append(single_con["A2Bbytes"])
          record_pktNum_list.append(single_con["A2Bpacket"])

    elif(flow_type == "total"):
  ...
  #write data
  fdata_d3js = open("./data/d3js_throughput.csv", "w")

  fdata_d3js.write("date,throughput,pkt_num,time_range,type\n")
  ...
  for i in range(len(record_date_list)):
    ...
    fdata_d3js.write(",")
    fdata_d3js.write(timeRange_str)
    fdata_d3js.write(",")
    fdata_d3js.write(flow_type)
    fdata_d3js.write("\n")
